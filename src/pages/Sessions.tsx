...
const Sessions = () => {
  ...
  const [showOnlyWithSql, setShowOnlyWithSql] = useState(false);

  const hasSql = (transformationInstance: any) => {
    // console.log(transformationInstance?.attributes?.some((attr: any) => attr.name === "Pre SQL" || attr.name === "Post SQL"))
    return transformationInstance?.attributes?.some((attr: any) => attr.name === "Pre SQL" || attr.name === "Post SQL");
  ...
  const validSessions =
    data?.sessions?.filter((s) => s.isValid === "YES").length || 0;
  const reusableSessions =
    data?.sessions?.filter((s) => s.reusable === "YES").length || 0;
        ...
        <div className="flex items-center space-x-4 mb-6">
          ...
          <div>
            ...
            <div className="flex items-center space-x-2 mt-4">
              <input type="checkbox" id="showOnlyWithSql" checked={showOnlyWithSql} onChange={(e) => setShowOnlyWithSql(e.target.checked)} className="rounded" />
              <label htmlFor="showOnlyWithSql" className="text-sm text-slate-600">Mostrar solo targets/sources con Pre/Post SQL</label>
            </div>
          </div>
        </div>
        ...
        {filteredSessions && filteredSessions.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredSessions.map((session) => {
              ...
              const allSourceNames = mappedMapping?.instances.filter(inst => inst.type === "SOURCE").map(inst => inst.name) || [];
              const allTargetNames = mappedMapping?.instances.filter(inst => inst.type === "TARGET").map(inst => inst.name) || [];
              ...
              const filteredSourceNames = showOnlyWithSql ? allSourceNames.filter(name => {
                const ti = session.transformationInstances?.find(ti => ti.instanceName === name && ti.transformationType === "Source Definition");
                return ti && hasSql(ti);
              }) : allSourceNames;

              const filteredTargetNames = showOnlyWithSql ? allTargetNames.filter(name => {
                const ti = session.transformationInstances?.find(ti => ti.instanceName === name && ti.transformationType === "Target Definition");
                (hasSql(ti) ?  console.log(ti) : '')
                return ti && hasSql(ti);
              }) : allTargetNames;
                    ...
                    <div className="grid grid-cols-3 gap-2 text-xs">
                     <div className="bg-blue-50 rounded-lg p-2 text-center">
                       ...
                       <p className="text-blue-900 font-bold">
                         {filteredSourceNames.length}
                       </p>
                     </div>
                     <div className="bg-green-50 rounded-lg p-2 text-center">
                       <p className="text-green-600 font-medium">Targets</p>
                       <p className="text-green-900 font-bold">
                         {filteredTargetNames.length}
                       </p>
                     </div>
                      <div className="bg-purple-50 rounded-lg p-2 text-center">
                        <p className="text-purple-600 font-medium">Transf.</p>
                        <p className="text-purple-900 font-bold">
                          {transformationNames.length}
                        </p>
                      </div>
                    </div>

                    {filteredSourceNames.length > 0 && (
                      <div className="p-3 bg-blue-50 rounded-lg">
                        <p className="text-xs text-blue-600 font-medium mb-1">
                          Sources ({filteredSourceNames.length})
                        </p>
                        <p className="text-sm text-blue-800">
                          {filteredSourceNames.slice(0, 2).join(", ")}
                          {filteredSourceNames.length > 2 ? "..." : ""}
                        </p>
                      </div>
                    )}
                    ...
                    {filteredTargetNames.length > 0 && (
                      <div className="p-3 bg-green-50 rounded-lg">
                        <p className="text-xs text-green-600 font-medium mb-1">
                          Targets ({filteredTargetNames.length})
                        </p>
                        <p className="text-sm text-green-800">
                          {filteredTargetNames.slice(0, 2).join(", ")}
                          {filteredTargetNames.length > 2 ? "..." : ""}
                        </p>
                      </div>
                    )}
                  </div>

                  <Link title="" to={`/powercenter/sessions/${session.name}`}>
